...
class base_Transformer(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        self.base = TransformerBase(img_size=14, patch_size=1, in_c=2048*3, embed_dim=768, depth=12, num_heads=12)
...
class base_Transformer1(nn.Module):
    def __init__(self, arch='resnet50'):
        super(base_Transformer1, self).__init__()

        #self.base = TransformerBase(img_size=224, patch_size=16, in_c=192, embed_dim=768, depth=12, num_heads=12)
        self.base1 = TransformerBase(img_size=14, patch_size=1, in_c=2048, embed_dim=256, depth=12, num_heads=8)

    def forward(self, x):
        x = self.base1(x)
        return x

class base_Transformer2(nn.Module):
    def __init__(self, arch='resnet50'):
        super(base_Transformer2, self).__init__()
...
class base_Transformer3(nn.Module):
    def __init__(self, arch='resnet50'):
        super(base_Transformer3, self).__init__()
...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local= 'on', gm_pool = 'on', arch='transformer'):
        ...
        self.base_Transformer = base_Transformer(arch=arch)
        self.base_Transformer1 = base_Transformer1(arch=arch)
        self.base_Transformer2 = base_Transformer2(arch=arch)
        self.base_Transformer3 = base_Transformer3(arch=arch)
        ...
        self.proj_2048_256 = nn.Linear(2048, 256, bias=False)

        pool_dim = 768
        #pool_dim = 2048
        self.l2norm = Normalize(2)
        self.bottleneck_x = nn.BatchNorm1d(pool_dim)
        self.bottleneck_y = nn.BatchNorm1d(pool_dim)
    ...
    def _pool_2048(self, t):
        # t: [B,2048,14,14]
        v = self.avgpool(t).view(t.size(0), t.size(1))
        return v
    def forward(self, x1, x2, x3, modal=0): #三模态记得modal=0,再加上
        if modal == 0:
            x1 = self.visible_module(x1)
            x2 = self.thermal_module(x2)
            x3 = self.depth_module(x3)
            y1 = self.base_Transformer1(x1)
            y2 = self.base_Transformer2(x2)
            y3 = self.base_Transformer3(x3)
            x_ori = None
            x_miss = None
            y_ori = None
            y_miss = None
            ...
            x1 = self.visible_module(x1)
            x2 = self.thermal_module(x2)
            x_ori = self.depth_module(x3)
            y1 = self.base_Transformer1(x1)
            y2 = self.base_Transformer2(x2)
            y_ori = self.base_Transformer3(x_ori)
            x3 = (x1+x2)/2
            y3 = (y1+y2)/2
            x_miss = x3
            y_miss = y3
            ...
            x1 = self.visible_module(x1)
            x3 = self.depth_module(x3)
            x_ori = self.thermal_module(x2)
            y1 = self.base_Transformer1(x1)
            y_ori = self.base_Transformer2(x_ori)
            y3 = self.base_Transformer3(x3)
            x2 = (x1+x3)/2
            y2 = (y1+y3)/2
            x_miss = x2
            ...
            x2 = self.thermal_module(x2)
            x3 = self.depth_module(x3)
            x_ori = self.visible_module(x1)
            y_ori = self.base_Transformer1(x_ori)
            y2 = self.base_Transformer2(x2)
            y3 = self.base_Transformer3(x3)
            x1 = (x2+x3)/2
            y1 = (y2+y3)/2
        ...
        if self.gm_pool == 'on':
            ...
            # 在幂运算前，确保x_flat没有负数
            y_flat = torch.abs(y)
            y_flat = y_flat + 1e-12  # 避免数值为0，防止除以0或出现log(0)的情况
            # 使用mean函数计算均值
            mean_values = torch.mean(y_flat ** p, dim=-1)
            # 确保mean_values不为零
            mean_values = torch.where(mean_values == 0, torch.ones_like(mean_values) * 1e-12, mean_values)
            y_pool = (mean_values) ** (1 / p)
        else:
            x_pool = self.avgpool(x)
            x_pool = x_pool.view(x_pool.size(0), x_pool.size(1))

            y_pool = self.avgpool(y)
            y_pool = y_pool.view(y_pool.size(0), y_pool.size(1))

        #print("x_pool before bottleneck:", x_pool)
        feat = self.bottleneck_x(x_pool)
        feat_y = self.bottleneck_y(y_pool)
        #print("Feature before classification:", feat, feat.shape)

        # x_ori/x_miss: [B,2048,14,14] -> [B,2048]
        if isinstance(x_ori, torch.Tensor):
            x_ori = self.avgpool(x_ori).view(x_ori.size(0), -1)
        if isinstance(x_miss, torch.Tensor):
            x_miss = self.avgpool(x_miss).view(x_miss.size(0), -1)

        # 可选：投影到 256（推荐）
        if not hasattr(self, "proj_2048_256"):
            self.proj_2048_256 = nn.Linear(2048, 256, bias=False).to(x1.device)
        if isinstance(x_ori, torch.Tensor):
            x_ori = self.proj_2048_256(x_ori)
        if isinstance(x_miss, torch.Tensor):
            x_miss = self.proj_2048_256(x_miss)

        # y_ori/y_miss: [B,196,256] -> [B,256]
        if isinstance(y_ori, torch.Tensor):
            y_ori = y_ori.mean(dim=1)
        if isinstance(y_miss, torch.Tensor):
            y_miss = y_miss.mean(dim=1)

        if self.training:
            return x_1, self.classifier(feat), y_1, self.classifier(feat_y), self.classifier(feat + feat_y), x_ori, x_miss, y_ori, y_miss
        else:
            return self.l2norm(x_pool), self.classifier(feat + feat_y), self.classifier(feat), self.classifier(feat_y)