...
class visible_module(nn.Module):
    def __init__(self, model_name="/16T/yzc/dinov3-vitb16-pretrain-lvd1689m", freeze_backbone=True, use_lora=False):
        ...
        if use_lora:
            lora_config = LoraConfig(
                r=8,
                lora_alpha=16,
                target_modules=["q_proj", "k_proj", "v_proj", "o_proj"],  # ViT 的注意力层
                lora_dropout=0.1,
                bias="none"
            )
...
class embed_net(nn.Module):
    def __init__(self, class_num=3, model_name="/16T/yzc/dinov3-vitb16-pretrain-lvd1689m", freeze_backbone=True, use_lora=False):
    ...
    def forward(self, x_v, x_t, x_d, model=0):
        ...
        if self.training:
            ...
            feat_raw = fused + x
            feat_norm = F.normalize(fused + x, p=2, dim=1)
            return feat_raw, logits_xy, logits_x, logits
